...
public class apriori {
	...
	public static class FrequentItemsMap extends Mapper<LongWritable, Text, Text, Text>
	{
		...
		public void map(LongWritable key,Text value,Context context) throws IOException
		{
		    ...
		    PropertyConfigurator.configure("/home/hadoop/sample/log/log4j.properties");
	...
	public static class AssociationRuleMap extends Mapper<LongWritable, Text, Text, Text>
	{
		...
		public void AssociationRuleRecursion(Vector<String> in, Vector<String> out, 
											int length, int level, int start,							
											Context context) throws IOException {
			...
			for(i=start;i<length;i++) {
				if(level==0){
				    ...
				    if(cntStr >= Support) {
				    	String str = "["+current.toString()+ "] => " +  in.toString() + " [X]-"+cntStr+" => [X,Y]-"+cntStr1+" CONFIDENCE-RATE =";
	...
	public static class FrequentItemsReduce extends Reducer<Text, Text, Text, Text>
	{
		...
		public void reduce(Text key,Iterable<Text> values,Context context) throws IOException
		{
			...
			try {
				logger.info("FrequentItemsReduce: Adding to DB FreqItems (Key,Qualifier,Value) => ("+key.toString()+","+key.toString()+","+Integer.toString(sum)+")");
	...
	public static class CandidateGenReduce extends Reducer<Text, Text, Text, Text>
	{
		...
		public void reduce(Text key,Iterable<Text> values,Context context) throws IOException
		{
			...
			try {
				logger.info("CandidateGenReduce: Adding to DB Candidates (Key,Qualifier,Value) => ("+key.toString()+","+key.toString()+","+Integer.toString(sum)+")");
				context.write(key, new Text(Integer.toString(sum)));
				p.add(Bytes.toBytes("count"), Bytes.toBytes(key.toString()),Bytes.toBytes(Integer.toString(sum)));
				context.progress();
			} catch (InterruptedException e) {
	...
	public static class AssociationRuleReduce extends Reducer<Text, Text, Text, Text>
	{
		...
		public void reduce(Text key,Iterable<Text> values,Context context) throws IOException
		{
			...
			try {
				logger.info("AssociationRuleReduce: Adding to DB Candidates (Key,Qualifier,Value) => ("+key.toString()+","+key.toString()+","+Integer.toString(sum)+")");
